...
:uri-originalplugin: https://github.com/danyill/oscd-tp-multicast-naming
:uri-plugin: https://github.com/aus-betz/oscd-tp-multicast-naming
...
:uri-openscd-core-plugin-demo: https://aus-betz.github.io/oscd-tp-multicast-naming/index.deploy.html
:source-highlighter: highlight.js

== What is this?

This plugin has been forked from the {uri-originalplugin}[Transpower] plugin of the same name and the intention is to modify it to meet the requirements of my employer. THis will include sequential autonumbering of the MAC addresses (unchanged per TP code), and possibly modify the APPID based on certain criteria. The GOOSE AppID (aka GoID) shall remain untouched by the plugin (completed), though I may create some further commands to enable bulk editing of chosen fields.
...
|GOOSE
|`ILock`, `TripCBFail`, `SPSBus`, `VReg`, `AdjBusCouplerInd`, `SwydLighting`
|1006-1049
|2006-2049
|e.g. Interlocking, SPS schemes across a bus, event triggering, bus protection CB Fail initiate, voltage regulation, indication across a bus section/coupler, switchyard lighting.
